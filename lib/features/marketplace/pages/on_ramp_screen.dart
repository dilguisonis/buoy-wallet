...
class OnRampOptionCard extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Future<void> launchIntegration(OnRampIntegration onRamp) async {
      if (onRamp.type == OnRampIntegrationType.guardarian) {
        final guardarianUri = Uri.parse('https://guardarian.com/calculator/v1').replace(
          queryParameters: {
      ...
      final uri = await ref.read(onRampOptionsProvider.notifier).formattedUri(onRamp);
      
      if (onRamp.type == OnRampIntegrationType.meld) {
        if (!context.mounted) return;
        await Navigator.of(context).push(
          MaterialPageRoute(
            builder: (context) => WebviewScreen(
    ...
    return BoxShadowCard(
      ...
      child: Material(
        ...
        child: InkWell(
          onTap: () async {
            await launchIntegration(integration);
          },
