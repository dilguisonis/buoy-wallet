...
final legacyBoltzProvider = Provider<BoltzService>((ref) {
...
final boltzSwapStatusStreamProvider =
    StreamProvider.family<BoltzSwapStatusResponse, SwapStatusRequest>(
        (ref, request) {
  return ref.read(legacyBoltzProvider).getLegacySwapStatusStream(request.id,
      forceNewStream: request.forceNewStream);
});
...
final boltzGetPairsProvider =
  ...
  return await ref.read(legacyBoltzProvider).getPairs();
});
...
class BoltzService {
  ...
  Future<BoltzCreateSwapResponse> createSwap({
    ...
    final baseUri =
        ref.read(legacyBoltzEnvConfigProvider.select((env) => env.apiUrl));
  ...
  @Deprecated('Only to support migration, use getSwapStatusStream instead')
  Stream<BoltzSwapStatusResponse> getLegacySwapStatusStream(String id,
  ...
  Future<BoltzBroadcastTransactionResponse> broadcastTransaction({
    ...
    final client = ref.read(dioProvider);
    const uri = '$boltzMainnetUrl/broadcasttransaction';
  ...
  Future<void> cacheRefundTx(
    ...
    await ref
        ...
        .updateRefundTxId(boltzId: swapId, txId: tx);
